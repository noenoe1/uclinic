        ...
        <template #header>
         ...
         <template>
            <div class="m-4 bg-background-100 dark:bg-backgrounddark-700 rounded-lg overflow-hidden">
                ...
                <ps-form-horizontal-section @submitted="searchBlogs"> 
                    <template #form>   
                        ...
                        <div class="flex justify-between w-full">    
                            <div class="flex items-center">
                                ...
                                <div class="relative text-gray-600 hidden sm:flex">
                                    <input type="search" name="serch" placeholder="Search blogs..." v-model="search" class="bg-background-500 dark:bg-backgrounddark-500 h-10 w-full xl:w-64 ps-5 pe-7 rounded-lg border border-background-300 dark:border-backgrounddark-700 text-sm focus:outline-none">
                                ...
                                <div class="mx-5">
                                    <PsButton class=" bg-primary-500 hover:bg-primary-700 active:bg-primary-100 me-2" id="search"> 
                                        Search
                                    </PsButton>
                                </div>
                ...
                <div class="" >
                    <table class="w-full ">
                        ...
                        <tbody class="container text-center">
                            ...
                            <tr v-for="(blog, index) in blogsresult.data" :key="blog.id" class=" shadow-sm"> 
                    ...
                    <nav class="w-full px-4 py-2 mt-2 items-end justify-end sm:px-6 inline-flex bg-background-300 dark:bg-backgrounddark-900 ">
                        <div v-for="link in blogsresult.links" :key="link.path" class="row">
    ...
    methods: {
        loadAllBlogs() {
            axios.get(route('blogs.getall')).then(response => {
                 ...
                 this.blogsresult = response.data;
                 } ).catch(error => console.log(error))
        },
        searchBlogs() {
            axios.get(route('blogs.search', {keywords: this.search})).then(response => {
                 this.blogsresult = response.data;
                 } ).catch(error => console.log(error))
        }
    }
