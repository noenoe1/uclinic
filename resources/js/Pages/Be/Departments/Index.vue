    ...
    <app-layout>
        ...
        <template #navbar>
            ...
            <ol class="flex items-center">
                ...
                <li class=" text-primary-500 me-3 text-sm">Departments List</li>                
            </ol>
            
        </template>

        <template #header>
            <h2 class="mx-4 mt-6 text-lg font-semibold"> Departments List </h2>
        </template>

         <template>
            <div class="m-4 bg-background-100 dark:bg-backgrounddark-700 rounded-lg overflow-hidden">
                
                <ps-form-horizontal-section @submitted="searchDepartments"> 
                    <template #form>   
                        ...
                        <div class="flex justify-between w-full">    
                            <div class="flex items-center">
                                ...
                                <div class="relative text-gray-600 hidden sm:flex">
                                    <input type="search" name="search" placeholder="Search departments..." v-model="search" class="bg-background-500 dark:bg-backgrounddark-500 h-10 w-full xl:w-64 ps-5 pe-7 rounded-lg border border-background-300 dark:border-backgrounddark-700 text-sm focus:outline-none">
                            ...
                            <div class="flex items-center">
                                    ...
                                    <PsButton class=" bg-primary-500 hover:bg-primary-700 active:bg-primary-100 me-2"> 
                                        <inertia-link href="/departments/create"> New Department </inertia-link>
                                    </PsButton>
                ...
                <div class="" >
                    <table class="w-full ">
                        <thead >
                            <tr class="border-b ">
                                ...
                                <th class="px-4 py-4">Department NAME</th>
                                <th class="px-4 py-4">Service NAME</th>
                        ...
                        <tbody class="container text-center">
                            ...
                            <tr v-for="(department, index) in deptsresult.data" :key="index" class=" shadow-sm"> 
                                <td class="px-4 py-4">{{++index}}</td>                
                                <td class="px-4 py-4">{{department.name}}</td>
                                <td class="px-4 py-4">{{department.service.name}}</td>
                                <td class="px-4 py-4">{{department.description}}</td>
                                <td class="py-2 py-4">
                                    <PsButton class=" bg-general-300"> 
                                        <inertia-link :href = "route('departments.view', {department: department})" > Edit </inertia-link>                                      
                                    </PsButton>
                                    <PsButton class=" bg-general-200"> 
                                        <inertia-link :href = "route('departments.delete', {department: department})" > Delete </inertia-link>                                      
                                    </PsButton>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <nav class="w-full px-4 py-2 mt-2 items-end justify-end sm:px-6 inline-flex bg-background-300 dark:bg-backgrounddark-900 ">
                        <div v-for="link in deptsresult.links" :key="link.path" class="row">
...
export default {
    ...
    methods: {
        loadAllDepartments() {
            axios.get(route('departments.getall')).then(response => {
                 console.log(response.data);
                 this.deptsresult = response.data;
                 } ).catch(error => console.log(error))
        },
        searchDepartments() {
            console.log('hello');
            
            axios.get(route('departments.search', {keywords: this.search})).then(response => {
                 console.log(" >>> ");  
                 console.log(response.data);
