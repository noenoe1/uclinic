    ...
    <app-layout>
        ...
        <template #navbar>
            ...
            <ol class="flex items-center">
                ...
                <li class=" text-primary-500 me-3 text-sm">Services List</li>                
            </ol>
            
        </template>

        <template #header>
            <h2 class="mx-4 mt-6 text-lg font-semibold"> Services List </h2>
        </template>

         <template>
            <div class="m-4 bg-background-100 dark:bg-backgrounddark-700 rounded-lg overflow-hidden">
                
                <ps-form-horizontal-section @submitted="searchServices"> 
                    <template #form>   
                        ...
                        <div class="flex justify-between w-full">    
                            <div class="flex items-center">
                                ...
                                <div class="relative text-gray-600 hidden sm:flex">
                                    <input type="search" name="serch" placeholder="Search services..." v-model="search" class="bg-background-500 dark:bg-backgrounddark-500 h-10 w-full xl:w-64 ps-5 pe-7 rounded-lg border border-background-300 dark:border-backgrounddark-700 text-sm focus:outline-none">
                            ...
                            <div class="flex items-center">
                                    ...
                                    <PsButton class=" bg-primary-500 hover:bg-primary-700 active:bg-primary-100 me-2"> 
                                        <inertia-link href="/services/create"> New Service </inertia-link>
                                    </PsButton>
                ...
                <div class="" >
                    <table class="w-full ">
                        ...
                        <tbody class="container text-center">
                            ...
                            <tr v-for="(service, index) in servsresult.data" :key="index" class=" shadow-sm"> 
                                <td class="px-4 py-4">{{++index}}</td> 
                                <td class="px-4 py-4"><img class="rounded-full h-20 w-20 object-cover" :src="'/storage/uploads/thumbnails/' + service.service_photo_path"></td>               
                                <td class="px-4 py-4">{{service.name}}</td>
                                <td class="px-4 py-4">{{service.description}}</td>
                                <td class="py-2 py-4">
                                    <PsButton class=" bg-general-300"> 
                                        <inertia-link :href = "route('services.view', {service: service})" > Edit </inertia-link>                                      
                                    </PsButton>
                                    <PsButton class=" bg-general-200"> 
                                        <inertia-link :href = "route('services.delete', {service: service})" > Delete </inertia-link>                                      
                                    </PsButton>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <nav class="w-full px-4 py-2 mt-2 items-end justify-end sm:px-6 inline-flex bg-background-300 dark:bg-backgrounddark-900 ">
                        <div v-for="link in servsresult.links" :key="link.path" class="row">
    ...
    methods: {
        loadAllServices() {
            axios.get(route('services.getall')).then(response => {
                 ...
                 this.servsresult = response.data;
                 } ).catch(error => console.log(error))
        },
        searchServices() {
            axios.get(route('services.search', {keywords: this.search})).then(response => {
                 this.servsresult = response.data;
                 } ).catch(error => console.log(error))
        }
    }
